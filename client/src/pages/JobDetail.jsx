...
const JobDetail = () => {
  ...
  const getJobDetails = async () => {
    ...
    try {
      ...
      if (user && job && job.application.includes(user._id)) {
        setIsApplied(true);
      }
    } catch (error) {
      setIsFetching(false);
  ...
  const handleDeletePost = async () => {
    ...
    try {
      if (window.confirm('Do you wante delete this post?')) {
        const res = await apiRequest({
  ...
  const handleApplyJob = async () => {
    ...
    try {
      if (window.confirm('Do you want to apply to this post?')) {
        const res = await apiRequest({
          url: `/jobs/apply-job/${job?._id}/${user?._id}`,
          token: user?.token,
          method: 'POST',
        });
        if (res?.success) {
          alert(res?.message);
          setIsApplied(true); // Defina o estado para true após a aplicação bem-sucedida
          // Armazene o estado isApplied no localStorage
  ...
  useEffect(() => {
    // Verifique se o usuário já se candidatou a este trabalho ao carregar a página
    if (job && user && job.application.includes(user._id)) {
      setIsApplied(true);
    } else {
      setIsApplied(false);
    }
  ...
  const handleCancelApplication = async () => {
    ...
    try {
      if (window.confirm('Deseja cancelar sua candidatura a este post?')) {
        ...
        if (res?.success) {
          alert(res?.message);
          setIsApplied(false); // Defina o estado para falso após o cancelamento bem-sucedido
  
          // Remova a chave do armazenamento local
          localStorage.removeItem(`applied_${job?._id}`);
        }
      }
      setIsFetching(false);
    } catch (error) {
        ...
        {isFetching ? (
          ...
          <div className="w-full h-fit md:w-2/3 2xl:2/4 bg-white px-5 py-10 md:px-10 shadow-md">
            ...
            <div className="my-6">
              {selected === '0' ? (
                <>
                  <p className="text-xl font-semibold">Job Deescription</p>
  ...
  <div className="flex items-center justify-center space-x-2">
    ...
    />
    <button
      className="w-full flex items-center justify-center text-white bg-gray-400 py-3 px-5 outline-none rounded-full text-base"
      disabled
    >
      Applied
    </button>
